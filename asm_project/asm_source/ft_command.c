...
void				check_text_2(char ***text, t_bot **bot, int *i)
{
	char			**a;
	...
	if (is_method(**text) == 1)
	{
		...
		(*bot)->keys = *ft_get_method(text, &(*bot)->hash_table,
&(*bot)->keys, i);
	}
	else
	{
...
unsigned int		*ft_command(char **text, int i, t_bot *bot)
{
	while (*text)
	{
		if (ft_strstr(*text, NAME_CMD_STRING) || ft_strstr(*text,
COMMENT_CMD_STRING))
